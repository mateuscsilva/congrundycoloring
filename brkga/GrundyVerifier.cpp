...
bool grundyConnectedSolutionVerifier(const GrundyInstance& instance, std::vector<int> nodesOrder, 
	std::vector<int> nodesColor){
	
	bool validSolution = true;
	int maxColor = -1;
	std::vector<int> vertexColor, inducedSubgraph;
	vertexColor.resize(instance.getNumNodes());
	...
	for(int i = 0; i < nodesOrder.size(); i++){
		...
		if(i > 0){
			validSolution = validSolution & 
				verifyNeighborhood(instance, nodesOrder[i], inducedSubgraph);
		}

		if(validSolution){
			vertexColor[nodesOrder[i]] = nodesColor[nodesOrder[i]];
			inducedSubgraph.push_back(nodesOrder[i]);
		}else{
			break;
		}
	}
