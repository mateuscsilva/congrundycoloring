...
function stdFormulation(inst::InstanceData, params::ParameterData,colors::Array{Int}, order::Array{Int}, 
	...
	elseif params.problem == "cgcol"
		stdFormulationConnectedGrundyColoring(inst::InstanceData, params::ParameterData, colors, order, zeta, psi)
	end
...
function repFormulation(inst::InstanceData, params::ParameterData, colors::Array{Int},order::Array{Int}, 
	...
	elseif params.problem == "cgcol"
		repFormulationConnectedGrundyColoring(inst::InstanceData, params::ParameterData, colors, order, zeta, psi)
	end
...
function repFormulationConnectedGrundyColoring(inst::InstanceData, params::ParameterData, colors::Array{Int}, order::Array{Int}, zeta, psi)
	println("Running Formulations.repFormulationConnected")
	model = defineModel(inst, params)


	maxdegree =  inst.maxdegree
	println("maxdegree = ",maxdegree)
	UB = min(maxdegree+1, zeta, maximum(psi))
	...
	### Defining variables ###
	@variable(model, x[u=1:inst.NV,v=u:inst.NV,t=1:inst.NV;u==v || u in antiG[v]], Bin)
	@variable(model, y[u=1:inst.NV,v=1:inst.NV;u!=v], Bin)


	### Objective function ###
	@objective(model, Max,
		sum(x[v,v,t] for v=1:inst.NV,t=1:inst.NV))
		

   	@constraint(model,
		time[t=1:inst.NV],
   		 sum(x[u,v,t] for u=1:inst.NV,v in antiGcol[u] if u<=v) == 1)		
	...
	@constraint(model,
		independentset[u=1:inst.NV,v in antiG[u], w in antiG[u]; u<v && v < w && w in inst.inputG[v]],
		sum(x[u,v,t] for t=1:inst.NV) + sum(x[u,w,t] for t=1:inst.NV) <= sum(x[u,u,t] for t=1:inst.NV))
	...
	for u=1:inst.NV,v in antiG[u], w in antiG[u]; u<v && v < w && w in inst.inputG[v]
   	...
   	@constraint(model,
		independentsetNew[u=1:inst.NV,v in antiG[u]; u<v],
   		 sum(x[u,v,t] for t=1:inst.NV) <= sum(x[u,u,t] for t=1:inst.NV))


	@constraint(model,
		iscovered[v=1:inst.NV],
   		sum(x[u,v,t] for u in antiG[v], t=1:inst.NV if u<=v ) <= 1)


	@constraint(model,
		grundypropuequalsv[u=1:inst.NV,v in antiGcol[u],p=1:inst.NV, t = 2:inst.NV;u <=v && u!=p],
		sum(x[u,v,tl] for tl = 1:t)  <= sum(x[p,w,tl] for w in inst.inputG[v], tl = 1:(t-1) if p in antiGcol[w] && p <= w) + 1 - y[p,u])


	@constraint(model,
		enforceprec[u in 1:inst.NV, v in u+1:inst.NV],
		y[v,u] + y[u,v] >=  sum(x[u,u,t] + x[v,v,t] for t=1:inst.NV) - 1)


	@constraint(model,
		precifrepreA[u in 1:inst.NV, v in u+1:inst.NV],
		y[u,v] + y[v,u] <=  sum(x[u,u,t] for t=1:inst.NV))

	@constraint(model,
		precifrepreB[u in 1:inst.NV, v in u+1:inst.NV],
		y[u,v] + y[v,u] <=  sum(x[v,v,t] for t=1:inst.NV))

	@constraint(model, connect[v in 1:inst.NV, t in 2:inst.NV], 
	sum(x[u,v,t] for u in antiGcol[v] if u <= v) <= sum(x[up, vp, tp] for vp in inst.inputG[v], tp in 1:t-1, up in antiGcol[vp] if up <= vp))	

	@constraint(model, limitRepresentatives, sum(x[u,u,t] for u in 1:inst.NV, t in 1:inst.NV) <= UB)
	...
	if params.initsol == 1
		...
		for t in 1:inst.NV
			for v in 1:inst.NV, k in 1:maxcol
				minindex = minimum(vwithcolor[k])
				if order[t] == v && k == colors[order[t]] && minindex <= v && (v in antiG[minindex] || v == minindex)
					set_start_value(x[minindex, v, t], 1)
				elseif minindex <= v && (v in antiG[minindex] || v == minindex)
					set_start_value(x[minindex, v, t], 0)	
				end
	...
	if nsolutions > 0
		...
		soldata = getSolutionFromREPCon(x, y, antiG, inst.NV)
		println(soldata)
		validate = validateGrundyConnectedSolution(soldata, inst.inputG)
...
function stdFormulationConnectedGrundyColoring(inst::InstanceData, params::ParameterData, colors::Array{Int}, order::Array{Int}, zeta, psi)
	println("Running Formulations.stdFormulation")
	model = defineModel(inst, params)


	maxdegree =  inst.maxdegree
	UB = min(maxdegree+1, maximum(psi), zeta)

	limitColorPerVertex, limitColorPerVertexWithTime, verticesCanBeColoredK = generateAuxiliaryStdConnectedFormulationStructs(inst, zeta, psi)
	

	### Defining variables ###
	@variable(model, w[k=1:UB], Bin)
	@variable(model, z[v=1:inst.NV, t=1:inst.NV, k=limitColorPerVertexWithTime[v][t]], Bin)
	...
	@constraint(model,
		independentset[edge=1:size(inst.edgeslist,1),k=intersect(limitColorPerVertex[(inst.edgeslist[edge,1])], limitColorPerVertex[(inst.edgeslist[edge,2])])],
  		sum(z[(inst.edgeslist[edge,1]),t,k] for t in k:inst.NV) + sum(z[(inst.edgeslist[edge,2]),t,k] for t in k:inst.NV) <= 1)


	@constraint(model,
		iscovered[v=1:inst.NV],
   		sum(z[v,t,k] for t=1:inst.NV, k=limitColorPerVertexWithTime[v][t]) == 1)


	@constraint(model,
	 onlyifused[k=1:UB],
	 w[k] <= sum(sum(z[v,t,k] for t=k:inst.NV) for v=1:inst.NV if k in limitColorPerVertex[v]))

	@constraint(model,
		adjcolors[v=1:inst.NV,k=1:(limitColorPerVertex[v][length(limitColorPerVertex[v])]-1),kp=k+1:limitColorPerVertex[v][length(limitColorPerVertex[v])],t=2:inst.NV],
		sum(z[v,tp,kp] for tp=kp:t) <= sum(z[u,tp,k] for u in intersect(inst.inputG[v], verticesCanBeColoredK[k]),tp=k:t-1))

	@constraint(model, connect[v=1:inst.NV, t=2:inst.NV], 
	sum(z[v,t,k] for k in limitColorPerVertexWithTime[v][t]) <= 
		sum(z[u,tp,k] for u in inst.inputG[v], tp in 1:t-1, k in limitColorPerVertexWithTime[u][tp]))	

	@constraint(model,
		onlyonefort[t=1:inst.NV],
		sum(z[v,t,k] for v=1:inst.NV,k=limitColorPerVertexWithTime[v][t]) == 1)
	...
	if params.initsol == 1
		...
		for t in 1:inst.NV
			vert = order[t]
			for v in 1:inst.NV, k in limitColorPerVertex[v]
				if colors[v] == k && vert == v
					kcolor[k] = 1
					set_start_value(z[v,t,k],1)
				elseif t >= k
					set_start_value(z[v,t,k],0)
				end
			end
		end
	...
	if nsolutions > 0
		...
		soldata = getSolutionFromSTDCon(z, w, UB, inst.NV, limitColorPerVertexWithTime)
		print(soldata)
		validate = validateGrundyConnectedSolution(soldata, inst.inputG)
...
function generateAuxiliaryStdConnectedFormulationStructs(inst, zeta, psi)

	limitColorPerVertex = Array{Vector{Int}}(undef,inst.NV)
	limitColorPerVertexWithTime = Vector{Vector{Vector{Int}}}(undef, inst.NV)
	verticesCanBeColoredK = Array{Vector{Int}}(undef,inst.maxdegree+1)
	...
	for v in 1:inst.NV
		auxLimit = Vector{Vector{Int}}(undef,inst.NV)
		for t in 1:inst.NV
			auxLimit[t] = [i for i = 1:min(length(inst.inputG[v])+1, zeta, psi[v], t)]
		end
		limitColorPerVertexWithTime[v] = auxLimit
	end
	...
	return limitColorPerVertex, limitColorPerVertexWithTime, verticesCanBeColoredK
end
