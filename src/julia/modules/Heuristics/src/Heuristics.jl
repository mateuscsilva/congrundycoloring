...
function connectGraph(inst)
	totalColors = 0
	color = [-1 for i = 1:inst.NV]
	queue = Queue{Int}()
	for i in 1:inst.NV
		if color[i] == -1
	...
	verticesToConnect = []
	for i in 1:totalColors
		max = -1
		index = -1
		for j in 1:inst.NV
			if color[j]==i && length(inst.inputG[j]) > max
				max = length(inst.inputG[j])
				index = j
			end
		end
		append!(verticesToConnect, index)
	end
	sort!(verticesToConnect)
	println(totalColors)
	println(verticesToConnect)
	newGraph = copy(inst.inputG)
	for i in 1:length(verticesToConnect)-1
		push!(newGraph[verticesToConnect[i]], verticesToConnect[i+1])
		push!(newGraph[verticesToConnect[i+1]], verticesToConnect[i])
	end

	open("graph_connected.col","a") do f
		for i in 1:inst.NV
			for j in newGraph[i]
				if i < j
					write(f, "e $(i) $(j)\n")
