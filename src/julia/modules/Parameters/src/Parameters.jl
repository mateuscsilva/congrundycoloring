...
struct ParameterData
    ...
    outputfile::String
    initsol::Int
    separationall::Int #whether separation of fractional cuts are performed in all nodes
    maxmaxcliques
    germansep::Int
    maxnodes::Int
    disablecliques::Int
...
function readInputParameters(ARGS)
    ...
    for param in 1:length(ARGS)
        ...
        elseif ARGS[param] == "--outputbound"
            ...
            param += 1
        elseif ARGS[param] == "--dzn"
            outputDzn = ARGS[param+1]
            param += 1
        end
    end

    params = ParameterData(instName,problem,method,form,solver,maxtime,tolgap,printsol,disablesolver,relaxation,outputfile,initsol,separationall,maxmaxcliques,germansep,maxnodes,disablecliques,germancliques,gamma,heurseed,maxsolutions, upperboundDepth, cuts, modelConfig, outputBound, outputDzn)
